...
Agent 配置本身两者完全相同：供 Codex、OpenCode、Cursor 使用的 vault 根目录 `AGENTS.md`，以及供 Claude Code 使用的 `.claude/CLAUDE.md`。两者都包含 Knowlery 的操作规则，并指向同样的两个来源——你的 `KNOWLEDGE.md` 和 `.agents/rules/` 下的 rules——而不复制它们：`AGENTS.md` 要求 agent 先读取这些文件（这些 harness 没有 import 语法），`CLAUDE.md` 则用 `@` 硬 import。`KNOWLEDGE.md` 始终是独立的一份。
...
| `.agents/rules/` | 所有平台共用的 rules（`AGENTS.md` 列出路径，`.claude/CLAUDE.md` 逐个 import） |
| `.claude/skills/` | 为 Claude Code 镜像的内置 skills |
| `AGENTS.md` | Codex、OpenCode 等的入口文件："Read First" 段指向 `KNOWLEDGE.md` 和每个 rule 文件，随后是 Knowlery 操作规则（受管区块，sync 时重新生成） |
