...
以及 Claude Code 上把 `knowlery` 放进 agent PATH 的 `bin/` shim。安装插件
即可替代上面的手动 MCP 配置。
...
- **Codex**：`codex plugin marketplace add JayJiangCT/knowlery` →
  `codex plugin add knowlery@knowlery`。安装后新建会话。在 Codex Desktop
  输入框键入 `@`，可选择插件或其中的技能；在 CLI 中可以直接描述任务，或用
  `$knowlery:ask` 这样的命名空间显式调用技能。
