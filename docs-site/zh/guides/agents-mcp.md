...
- **Claude Code**：把仓库加为插件源并安装 `knowlery`
  （`/plugin marketplace add <路径或仓库>` → `/plugin install knowlery`）。
  技能以 `/knowlery:<name>` 出现。
- **Codex**：通过 marketplace 条目添加插件
  （`codex plugin add knowlery@<marketplace>`）；技能用 `@knowlery` 调用。
- **Cursor**：从插件目录安装；MCP 工具和技能对 agent 注册。

Marketplace 上架（无需 checkout 的一键安装）随 1.1 的分发工作落地。插件
**不执行任何安装脚本**——MCP 供给就是配置 + npx，安装时刻没有代码运行。

**插件技能 vs vault 技能**：插件技能是会话级全局、带命名空间的
（`/knowlery:ask`）；Knowlery 工作区也带自己的副本（`/ask`）。两者从同一
...
| Agent 类型 | 1.0 的答案 |
| --- | --- |
| 本地 MCP 客户端（Claude Desktop/Code、Codex、Cursor） | `knowlery mcp` stdio——完整支持，全部九个工具 |
