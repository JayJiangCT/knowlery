...
`,
  },
  {
    ...
    kind: 'knowledge',
    emoji: '\u{1F517}',
    description: 'Bridge two topics using the vault link graph',
    content: `---
name: connect
description: >
...
`,
  },
  {
    ...
    kind: 'knowledge',
    emoji: '\u{1F30A}',
    description: 'Intention-vs-action gap analysis over time',
    content: `---
name: drift
description: >
...
`,
  },
  {
    ...
    kind: 'knowledge',
    emoji: '\u{1F50D}',
    description: 'Full vault structural health check',
    content: `---
name: mise
description: >
...
`,
  },
  {
    ...
    kind: 'knowledge',
    emoji: '\u{1F4CB}',
    description: 'Shared prerequisites check and graph enrichment',
    content: `---
name: prep
description: >
  Shared prerequisites check and graph enrichment. Scans frontmatter for cross-reference
  suggestions. Referenced by all skills via "(see /prep)".
...
`,
  },
  {
    ...
    kind: 'knowledge',
    emoji: '\u{1F4D6}',
    description: 'Generate and maintain INDEX.base knowledge map',
    content: `---
name: wiki
description: >
...
### Step 2: Index source (Bases first, markdown outline as fallback)

#### Step 3a — Bases / \`INDEX.base\` (preferred)

1. If the vault has no **\`INDEX.base\`** at the root, install the template from the Knowlery package.
...
### Step 3: Ensure \`INDEX.base\` matches that scope
...
1. **Global scope (recommended)** — Top-level **\`filters\`** with \`or:\` so every view inherits the same universe, e.g. \`file.inFolder("entities")\`, … \`file.inFolder("queries")\`.

2. **Formulas (optional but valuable)** — e.g. \`type_label\`, \`days_since_update\` from frontmatter \`updated\`, \`backlink_count\` from \`file.backlinks.length\`.

3. **\`properties\` + \`displayName\`** — Include **\`file.name\`** plus \`title\`, \`domain\`, \`tags\`, \`updated\`, \`status\`, and \`formula.*\` fields.

4. **Views (suggested six)** — All Pages (\`groupBy: type\`); Entities / Concepts (\`type\` filter + \`groupBy: domain\`); Comparisons; Queries; **Recently Updated** — use **\`limit\`** and put **\`updated\` first in \`order\`**.

5. **YAML** — Keep formula strings consistently quoted.

### Step 4: Present the index to the user
...
- **One index on disk:** **\`INDEX.base\`** when Bases is on. Chat markdown is fallback only.
...
`,
  },
  {
    ...
    kind: 'tooling',
    emoji: '\u{1F9E0}',
    description: 'Guide for vault-powered thinking workflows',
    content: `---
name: vault-thinking
