...
export async function readInstalledBundles(app: App): Promise<InstalledBundlesFile> {
  const path = normalizePath(BUNDLES_REGISTRY_PATH);
  if (!(await app.vault.adapter.exists(path))) return { schemaVersion: 1, bundles: {} };
  try {
    return InstalledBundlesFileSchema.parse(JSON.parse(await app.vault.adapter.read(path)));
  } catch {
    return { schemaVersion: 1, bundles: {} };
  }
}

export async function writeInstalledBundles(app: App, file: InstalledBundlesFile): Promise<void> {
  if (!(await app.vault.adapter.exists('.knowlery'))) await app.vault.adapter.mkdir('.knowlery');
  await app.vault.adapter.write(normalizePath(BUNDLES_REGISTRY_PATH), `${JSON.stringify(file, null, 2)}\n`);
}
